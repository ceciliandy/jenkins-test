...
APP_HOME=/home/jinen/jenkins/demo
  
#Java主程序,即main(String[] args)方法类
APP_MAIN=com.example.jenkins.jenkinstest.JenkinstestApplication
...
startup(){
	...
	then
		...
		nohup $JAVA_HOME/bin/java -cp ./lib/*:./dist/* $APP_MAIN > $APP_LOG/nohup.log &
        getTradeProtalPID
        if [ $tradePortalPID -ne 0 ]
		then
